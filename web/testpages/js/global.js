...
var loader = '<div class="loader">&nbsp;</div>', result = '<div class="result"></div>',
    status = '<div class="status"></div>', page = 'undefined', action = 'undefined';
...
function getRecommend(rec_type, cat_id) {
    $.post('index.php?act=cat_rec', {rec_type: rec_type, cid: cat_id}, function (response) {
...
function orderQuery() {
    ...
    if (order_sn.length < 10 || !reg.test(order_sn)) {
        ...
        $.get('user.php?act=order_query&order_sn=s', 'order_sn=s' + order_sn, function (response) {
...
function submitVote() {
    ...
    if (option == '') {
        ...
        $.post('vote.php', {vote: vote_id, options: option, type: type}, function (response) {
            var res = $.evalJSON(response);
            if (res.message != '') {
...
function addEmailList() {
    ...
    if (!isValidEmail(email)) {
        ...
        $.get('user.php?act=email_list&job=add', 'email=' + email, function (response) {
            $('#subscription .result').css({
                display: 'block',
                backgroundColor: '#97cf4d'
            }).html(response).animate({backgroundColor: '#fff'}, 1000);
...
function cancelEmailList() {
    ...
    if (!isValidEmail(email)) {
        ...
        $.get('user.php?act=email_list&job=del', 'email=' + email, function (response) {
...
(function ($) {
    $.fn.ChangePriceSiy = function () {
        ...
        function loadPrice() {
            ...
            ;$.get('goods.php', 'act=price&id=' + goodsId + '&attr=' + attr + '&number=' + qty, function (response) {
                var res = $.evalJSON(response);
...
function buy(id, num, parent) {
    ...
    $.post('flow.php?step=add_to_cart', {goods: $.toJSON(goods)}, function (response) {
        var res = $.evalJSON(response);
        if (res.error > 0) {
            if (res.error == 2) {
...
function collect(id) {
    $.get('user.php?act=collect', 'id=' + id, function (response) {
        var res = $.evalJSON(response);
        $.fn.colorbox({html: '<div class="message_box mb_info">' + res.message + '<p class="action"><a href="javascript:void(0);" class="button brighter_button" onclick="$.fn.colorbox.close(); return false;"><span>' + lang.confirm + '</span></a></p></div>'});
    }, 'text');
}
...
function addPackageToCart(id) {
    ...
    $.post('flow.php?step=add_package_to_cart', {package_info: $.toJSON(package_info)}, function (response) {
        var res = $.evalJSON(response);
        if (res.error > 0) {
            if (res.error == 2) {
...
function validLogin() {
    $('#username_login, #password_login').valid8('').tipsy({
        gravity: 'w',
        fade: true,
        trigger: 'manual'
    }).focusout(function () {
    ...
    if ($('#captcha_login').length > 0) {
        $('#captcha_login').valid8('').next().tipsy({
            gravity: 'w',
            fade: true,
            trigger: 'manual'
        }).attr('original-title', lang.error_captcha_required);
        $('#captcha_login').focusout(function () {
            validAndTipNext($(this));
        }).keyup(function () {
            validAndTipNext($(this));
        });
    }
...
function openQuickLogin() {
    $.fn.colorbox({
        scrolling: false, href: 'user.php?act=login&ajax=1', onComplete: function () {
            $('.tipsy').remove();
            validQuickLogin();
        }, onCleanup: function () {
            $('.tipsy').remove();
...
function validQuickLogin() {
    ...
    $('#username_login, #password_login').tipsy({gravity: 'w', fade: true, trigger: 'manual'}).focusout(function () {
        validAndTip2($(this));
    }).keyup(function () {
        validAndTip2($(this));
    });
    ...
    $('#password_login').attr('original-title', lang.error_password_required);
    if ($('#captcha_login').length > 0) {
        $('#captcha_login').next().tipsy({
            gravity: 'w',
            fade: true,
            trigger: 'manual'
        }).attr('original-title', lang.error_captcha_required);
        $('#captcha_login').focusout(function () {
            validAndTipNext2($(this));
        }).keyup(function () {
            validAndTipNext2($(this));
        });
    }
...
function submitComment(frm) {
    ...
    validItem.tipsy({gravity: 's', fade: true, trigger: 'manual'}).valid8('').focusout(function () {
        validAndTip($(this));
    }).keyup(function () {
        validAndTip($(this));
    });
    ...
    $.post('comment.php', {cmt: $.toJSON(cmt)}, function (response) {
        var res = $.evalJSON(response);
        if (res.message) {
            $.fn.colorbox({html: '<div class="message_box mb_info">' + res.message + '<p class="action"><a href="javascript:void(0);" class="button brighter_button" onclick="$.fn.colorbox.close(); return false;"><span>' + lang.confirm + '</span></a></p></div>'});
        }
...
function gotoPage(page, id, type) {
    $.get('comment.php?act=gotopage', 'page=' + page + '&id=' + id + '&type=' + type, function (response) {
        var res = $.evalJSON(response);
        $('#comment_wrapper').html(res.content);
        $('.rank_star').rating({
            focus: function (value, link) {
...
function gotoBuyPage(page, id) {
    $.get('goods.php?act=gotopage', 'page=' + page + '&id=' + id, function (response) {
        var res = $.evalJSON(response);
...
function sendHashMail() {
    $.get('user.php?act=send_hash_mail', '', function (response) {
        var res = $.evalJSON(response);
        $.fn.colorbox({html: '<div class="message_box mb_info">' + res.message + '<p class="action"><a href="javascript:void(0);" class="button brighter_button" onclick="$.fn.colorbox.close(); return false;"><span>' + lang.confirm + '</span></a></p></div>'});
        $('#comment_wrapper').html(res.content);
    }, 'text');
...
function bid() {
    ...
    if (price == '') {
        ...
        if (!reg.test(price)) {
            ...
            $.post('snatch.php?act=bid', {id: id, price: price}, function (response) {
                var res = $.evalJSON(response);
                if (res.error == 0) {
...
function loadRegions(parent, type, target) {
    ...
    $.get('region.php', 'type=' + type + '&target=' + target + "&parent=" + parent, function (response) {
        var res = $.evalJSON(response);
...
function loadCart(show) {
    $.post('flow.php?step=cart&ajax=1', '', function (response) {
        $('#cart .loader').fadeTo(1000, 0);
        $("#cart").html(response);
        if (show == 1) {
            $('#cart .list_wrapper').css({
                display: 'block',
                backgroundColor: '#ffc'
...
function cartDrop(id) {
    ...
    $.get('flow.php?step=drop_goods', 'id=' + id, function (response) {
        if ($('#page_flow').length > 0) {
            if (action == 'checkout') {
...
function cAlert(content) {
    $.fn.colorbox({
        transition: 'none',
        html: '<div class="message_box mb_info">' + content + '<p class="action"><a href="javascript:void(0);" class="button brighter_button" onclick="$.fn.colorbox.close(); return false;"><span>' + lang.confirm + '</span></a></p></div>'
    });
}
...
function submitTag() {
    ...
    if (tag.length > 0 && parseInt(goods_id) > 0) {
        $.post('user.php?act=add_tag', {id: goods_id, tag: tag}, function (response) {
            var res = $.evalJSON(response);
            if (res.error > 0) {
                cAlert(res.message);
            } else {
...
function gotoPageGrade(page, id, type, grade) {
    ...
    if (commentsGrade.attr('page') != page) {
        ...
        $.get('siy_comment.php?act=gotopage', 'page=' + page + '&id=' + id + '&type=' + type + '&grade=' + grade, function (response) {
            var res = $.evalJSON(response);
            var commentsGrade = $('#comments_' + grade);
...
function gotoPageQuestion(page, id, type, cat) {
    ...
    if (questionsCat.attr('page') != page) {
        loader.css({visibility: 'visible'}).fadeTo(0, 1000);
        $.get('siy_question.php?act=gotopage', 'page=' + page + '&id=' + id + '&type=' + type + '&cat=' + cat, function (response) {
            var res = $.evalJSON(response);
            var questionsCat = $('#questions_' + cat);
...
function siyQuickReply(cid, page, id, type) {
    $.fn.colorbox({
        ...
        height: '90%',
        width: '700px',
        href: '../admin/comment_manage.php?act=reply&id=' + cid,
        onCleanup: function () {
...
function packagePickOut(package_id) {
    ...
    $.post('flow.php?step=package_pick_out', {
...
function likeArticle(id) {
    if ($('.current-like').hasClass('liked')) {
        ...
        $.get('article.php?act=like', 'id=' + id, function (response) {
            var res = $.evalJSON(response);
            if (res.error > 0) {
                $.fn.colorbox({html: '<div class="message_box mb_info">' + res.message + '<p class="action"><a href="javascript:void(0);" class="button brighter_button" onclick="$.fn.colorbox.close(); return false;"><span>' + lang.confirm + '</span></a></p></div>'});
            } else {
